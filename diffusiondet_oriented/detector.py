...
from .util.box_ops import box_cxcywh_to_xyxy, box_xyxy_to_cxcywh, box_xyxy_to_corners,box_corners_to_cxcywha,box_cxcywha_to_corners
...
class DiffusionDet(nn.Module):
    ...
    def prepare_diffusion_concat(self, gt_boxes):
        ...
        # Create noise tensor with shape (num_proposals, 8)
        noise = torch.randn(self.num_proposals, 8, device=self.device)
    
        num_gt = gt_boxes.shape[0]
        if not num_gt:  # generate fake gt boxes if empty gt boxes
            gt_boxes = torch.as_tensor([[0., 0., 1., 0., 1., 1., 0., 1.]], dtype=torch.float, device=self.device)
            num_gt = 1
    
        if num_gt < self.num_proposals:
            box_placeholder = torch.randn(self.num_proposals - num_gt, 8,
...
#Note that this normalization assumes that the corner coordinates are provided in a consistent order (e.g., top-left, top-right, bottom-right, bottom-left). Make sure that the corner coordinates in your dataset follow the  expected order for this normalization to work correctly.
    ...
    def inference(self, box_cls, box_pred, image_sizes):
        ...
        if self.use_focal or self.use_fed_loss:
            ...
            # For each box we assign the best class or the second best if the best one is `no_object`.
